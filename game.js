...
// === INIT === document.addEventListener('DOMContentLoaded', () => { loadGame(); updateMassDisplay(); setupUI(); startGameLoop();

fetch('data/upgrades.json') .then(response => response.json()) .then(data => { gameState.availableUpgrades = data; renderUpgrades(); });

startTutorial(); setupPrestigeButton(); });

// === PARTICLE CLASS === class Particle { constructor(x, y, dx, dy, passive = false) { this.x = x; this.y = y; this.dx = dx; this.dy = dy; this.size = 4; this.passive = passive; }

move() { this.x += this.dx; this.y += this.dy; }

draw() { CTX.beginPath(); CTX.fillStyle = this.passive ? '#888' : '#00d8ff'; CTX.arc(this.x, this.y, this.size, 0, Math.PI * 2); CTX.fill(); }

checkCollision() { const dist = Math.hypot(this.x - CENTER.x, this.y - CENTER.y); if (dist < 15) { const multiplier = 1 + (getUpgradeLevel('density') * 0.1) + (gameState.darkEnergy * 0.05); gameState.mass += MASS_PER_CLICK * multiplier; updateMassDisplay(); return true; } return false; } }

// === GAME LOOP === function startGameLoop() { setInterval(() => { const now = Date.now(); const delta = now - gameState.lastUpdate; gameState.lastUpdate = now;
...
// === UPGRADE SYSTEM === function renderUpgrades() { const list = document.getElementById('upgrade-list'); list.innerHTML = '';

gameState.availableUpgrades.forEach(upg => { const level = getUpgradeLevel(upg.id); const cost = calculateUpgradeCost(upg, level);
...
function purchaseUpgrade(upg) { const level = getUpgradeLevel(upg.id); const cost = calculateUpgradeCost(upg, level); if (gameState.mass >= cost && level < upg.maxLevel) { gameState.mass -= cost; gameState.upgrades[upg.id] = level + 1; updateMassDisplay(); renderUpgrades(); saveGame(); } }

function calculateUpgradeCost(upg, level) { return Math.floor(upg.baseCost * Math.pow(upg.costMultiplier, level)); }
...
function saveGame() { localStorage.setItem('collidle-save', JSON.stringify(gameState)); }
